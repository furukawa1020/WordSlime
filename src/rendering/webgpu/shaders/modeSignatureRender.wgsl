...
fn fs_main(input: VertexOut) -> @location(0) vec4f {
  ...
  if (mode < 0.5) {
    ...
    let blob =
      ...
      (0.24 + fbm(warped * 4.2 + time * 0.08) * 0.12);
    ...
    color = vec3f(0.28, 0.95, 0.8) * membrane + vec3f(0.08, 0.38, 0.32) * inner + vec3f(0.65, 1.0, 0.95) * sheen * 0.22;
    ...
    let arc = pow(max(0.0, sin(angle * 3.0 - time * 1.2)), 12.0) * circle_line(distance, 0.32, 0.09);
    let cells = floor((uv + vec2f(time * 0.018, -time * 0.01)) * vec2f(38.0, 22.0));
    let spark = step(0.984, hash(cells)) * smoothstep(0.74, 0.12, distance);
    ...
    let plume = smoothstep(0.86, 0.18, abs(uv.x - 0.5)) * smoothstep(1.0, 0.28, uv.y);
    let layer1 = fbm(vec2f(uv.x * 5.0 + time * 0.06, uv.y * 9.0 - time * 0.15));
    let layer2 = fbm(vec2f(uv.x * 12.0 - time * 0.03, uv.y * 5.2 + time * 0.08));
    let vapor = plume * smoothstep(0.38, 0.86, layer1) * (0.45 + layer2 * 0.55);
    ...
    let branch_a = pow(1.0 - abs(sin(angle * 6.0 + distance * 17.0 - time * 0.28)), 9.0);
    let branch_b = pow(1.0 - abs(sin(angle * 11.0 - distance * 24.0 + time * 0.18)), 12.0);
    ...
    let block = step(0.78, hash(block_cell));
    let tear = pow(max(0.0, sin((uv.x + fbm(uv * 8.0)) * 40.0 + time * 8.0)), 18.0);
